 ...
 *               (start $boom)
 *               (func (export "size") (result i32) table.size 0))
 *   elemoob:  (module (import "e" "t" (table 1 funcref))
 ...
 * so.
 *
 * The mark also has a MOMENT, not just a place, and the criterion for it is
 * REACHABILITY: the SOURCE is marked at the line that PUBLISHES `rt.tables`,
 * because that is the line at which the binder's aliasing copy becomes
 * readable by anything other than the instantiation still building it. The
 * copy itself is marked as it is made — a start function must not grow
 * through it — but the source waits for the publish, since the binder runs
 * before `checkImportTypeMatches` and before the runtime exists, and a mark
 * set there would cost a host table its growth for an importer that never
 * came to exist.
 *
 * Four scenarios stand on the two sides of that line. Before it, the copy
 * exists but nobody can reach it, and the host table stays growable:
 * `checkImportTypeMatches` refuses the import (7), or a second, module-defined
 * table fails its initial allocation after `tbl_storage` is filled and before
 * it is published (10). After it, the instantiation fails with a runtime that
 * is parked rather than unwound (ADR-0014 §2.1) and still holds the copy, so
 * the source must decline: an out-of-bounds active element segment (9), or a
 * trapping start function (8).
 *
 * Nine things are measured:
 *
 ...
 *   9. an instantiation that fails EARLIER than the start — on an out-of-bounds
 *      active element segment — also still declines the grow. `rt.tables` is
 *      filled before the element segments are applied, so the failure parks a
 *      runtime that already holds the copy (ADR-0014 §2.1) even though no start
 *      function ever ran. A commit point after `instantiateRuntime` RETURNS
 *      would miss this: measured on both engines with the mark written there,
 *      the host's grow succeeded and left the parked copy on a freed buffer.
 *      Measured on both engines here: the instance is NULL and no trap comes
 *      out — the failure is an instantiation error, not a guest trap.
 ...
 * scenarios (1, 2, 5, 7, 8 and 9) run on `auto` and `interp` only — an engine that
...
/* wasm-tools parse + strip --all of the `elemoob` module above. The import
 * matches, so the copy is installed into `rt.tables`; the first element segment
 * then writes slot 0 in bounds and the second, at offset 5 of a 1-cell table,
 * fails the instantiation — past the install, before any start function.
 * Exports: none. */
static const unsigned char kElemOobWasm[] = {
    0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x05, 0x01, 0x60,
    0x00, 0x01, 0x7f, 0x02, 0x09, 0x01, 0x01, 0x65, 0x01, 0x74, 0x01, 0x70,
    0x00, 0x01, 0x03, 0x02, 0x01, 0x00, 0x09, 0x0d, 0x02, 0x00, 0x41, 0x00,
    0x0b, 0x01, 0x00, 0x00, 0x41, 0x05, 0x0b, 0x01, 0x00, 0x0a, 0x06, 0x01,
...
/* 9. A LATE instantiation failure that is not the start function: the second
 * active element segment is out of bounds. `instantiateRuntime` installs the
 * imported table copy into `rt.tables` and only THEN applies the element
 * segments (and the data segments, the globals and the exports after them), so
 * this failure lands with the aliasing copy already live and the runtime parked
 * (ADR-0014 §2.1) — the same state scenario 8 reaches, one stage earlier and
 * without a start function.
 *
 * The mark must therefore be committed INSIDE `instantiateRuntime` at the
 * install, not after it returns: written after the return, this path never
 * reaches it, and the host's grow then reallocs the backing the parked copy
 * still points at. Measured with the mark written after the return, the grow
 * below returned true.
 *
 * No trap comes out of this one — an out-of-bounds active element segment is an
 * instantiation error, not a guest trap — so the NULL instance is the whole
 * signal; the out-param is drained defensively in case that ever changes. */
static int late_instantiate_failure_still_declines_grow(uint8_t engine) {
    int rc = 1;
    const char* who = engine_name(engine);
    const char* what = "late instantiate failure";
    wasm_table_t* ht = NULL;
    wasm_module_t* module = NULL;
    wasm_instance_t* instance = NULL;
    wasm_engine_t* eng = wasm_engine_new();
    ...
    wasm_byte_vec_t binary = { sizeof(kElemOobWasm), (wasm_byte_t*) kElemOobWasm };
    module = wasm_module_new(store, &binary);
    if (!module) { fprintf(stderr, "[%s] %s: the guest failed to parse\n", who, what); goto cleanup; }
    wasm_extern_t* import_externs[1] = { wasm_table_as_extern(ht) };
    wasm_extern_vec_t imports = { 1, import_externs };
    ...
    if (itrap) wasm_trap_delete(itrap);
    if (instance) {
        fprintf(stderr, "[%s] %s: the guest instantiated with an out-of-bounds element segment\n",
                who, what);
        goto cleanup;
    }

    /* The parked runtime still holds the copy, so the source stays declined. */
    if (wasm_table_grow(ht, kGrowDelta, NULL)) {
        fprintf(stderr, "[%s] %s: wasm_table_grow returned true after a parked instantiation\n",
                who, what);
        goto cleanup;
    }
    if (expect_handle_size(ht, kInitialSize, who, what, "after the declined grow") != 0) goto cleanup;
...
int main(void) {
    for (size_t i = 0; i < sizeof(kImportEngines) / sizeof(kImportEngines[0]); i++) {
        ...
        if (late_instantiate_failure_still_declines_grow(kImportEngines[i]) != 0) return 1;
    }
