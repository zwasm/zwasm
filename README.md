...
| Wasm 3.0 (GC, EH, tail-call, memory64, multi-memory, typed func refs, extended-const, relaxed-simd, custom annotations) | ✅ 100% | all 9 proposals, on the 3-OS CI matrix: the interpreter lane is green; the JIT lane is green except 8 enumerated `try_table` f32/f64 directives (#378), which its known list fails the run over if they change (extended-const: unit-tested, its corpus lane is pending #217) |
...
| WASI 0.2 (preview2, Component Model) | ✅ functional, default-ON | real `wasm32-wasip2` Rust / TinyGo components run end-to-end (fs, sockets incl. TCP listeners, guest-defined resources); typed embedder API (introspection + `invokeTyped`); the Component Model spec corpus (incl. the official `assert_invalid` cases) is green in `test-all`; gated by `-Dwasi>=p2` (the default), `-Dwasi=p1` opts out |
| WASI 0.3 (preview3, native async)    | ✅ full coverage, opt-in (`-Dwasi=p3`) | all six WASI 0.3.0 proposals (released 2026-06-11) — `cli` / `clocks` / `random` / `filesystem` / `sockets` / `http` — on the Component Model's native async (`stream<T>` / `future<T>`, waitable-sets; async-lifted exports in the callback form, a stackful async lift is declined). Sockets (TCP and UDP) and http (client and the exported `handler`) are served end-to-end. Official `wasm32-wasip3` corpus **45/45 green on all three supported OSes** (`@unstable` interfaces excluded, as upstream gates them); the lane reconciles that count against the vendored corpus |
