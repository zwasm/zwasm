...
fn crossModuleJitTarget(
    ...
    // overflow rule (`computeCallOverflowBytesCc`) excludes v128, so the thunk
    // cannot size a copy for it. Decline the shape — NULL, as `v2.6.0`
    // returned for every cross-module import — rather than carry it wrongly.
...
/// (see `crossModuleJitTarget`).
fn hasV128Param(sig: zir.FuncType) bool {
    for (sig.params) |vt| if (vt == .v128) return true;
    return false;
}

test "hasV128Param: only a v128 parameter is declined; results and every scalar class are not (#390)" {
    const i32s = [_]zir.ValType{.i32} ** 12;
    const three = [_]zir.ValType{.i32} ** 3;
    try testing.expect(!hasV128Param(.{ .params = &i32s, .results = &three }));
    try testing.expect(!hasV128Param(.{ .params = &.{ .f64, .f32, .i64, .{ .ref = zir.RefType.abs(.func, true) } }, .results = &.{.v128} }));
