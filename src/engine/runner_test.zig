...
const CrossModuleHarness = struct {
    ...
    fn link(
        ...
        shared_thunk.emitThunk(slot, callee_rt, callee_entry, b_compiled.func_sigs[b_idx]);
...
// ADR-0228 / #390 — the bridge carries what the ABI does not pass in
// registers. The exporter's signatures overflow every convention's register
// file (SysV at the 6th int, Win64 at the 4th position, arm64 at the 8th int
// / 9th fp) and return MEMORY-class (three results); the importer pushes
// constants and folds. Every expected value is the interpreter's (the C-API
// sibling `cross_module_abi.c` runs `interp` as the oracle). Fixtures:
//
//   exporter: pick7 / sum7 (7 × i32), pick9 (9 × i32),
//             mixsum (6 × i32, 9 × f64 → f64), three (→ i32 i32 i32),
//             three_of (7 × i32 → i32 i32 i32: args 0, 3, 6)
//   importer: test_<name> — pushes 11..77 / 1..7 / 1..99 / powers of two,
//             folds three results as a*100 + b*10 + c
const abi_exporter_wasm = [_]u8{
    ,
};

const abi_importer_wasm = [_]u8{
    ,
};

const indirect_importer_wasm = [_]u8{
    ,
};

const glob_exporter_wasm = [_]u8{
    0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x05, 0x01, 0x60,
    0x00, 0x01, 0x7f, 0x03, 0x02, 0x01, 0x00, 0x06, 0x07, 0x01, 0x7f, 0x01,
    0x41, 0xe8, 0x07, 0x0b, 0x07, 0x08, 0x01, 0x04, 0x62, 0x75, 0x6d, 0x70,
    0x00, 0x00, 0x0a, 0x0d, 0x01, 0x0b, 0x00, 0x23, 0x00, 0x41, 0x01, 0x6a,
    0x24, 0x00, 0x23, 0x00, 0x0b, 0x00, 0x0b, 0x04, 0x6e, 0x61, 0x6d, 0x65,
    0x07, 0x04, 0x01, 0x00, 0x01, 0x67,
};

const glob_importer_wasm = [_]u8{
    0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x05, 0x01, 0x60,
    0x00, 0x01, 0x7f, 0x02, 0x0a, 0x01, 0x01, 0x65, 0x04, 0x62, 0x75, 0x6d,
    0x70, 0x00, 0x00, 0x03, 0x02, 0x01, 0x00, 0x06, 0x06, 0x01, 0x7f, 0x01,
    0x41, 0x07, 0x0b, 0x07, 0x08, 0x01, 0x04, 0x74, 0x65, 0x73, 0x74, 0x00,
    0x01, 0x0a, 0x10, 0x01, 0x0e, 0x00, 0x23, 0x00, 0x41, 0x01, 0x6a, 0x24,
    0x00, 0x10, 0x00, 0x1a, 0x23, 0x00, 0x0b, 0x00, 0x17, 0x04, 0x6e, 0x61,
    0x6d, 0x65, 0x01, 0x07, 0x01, 0x00, 0x04, 0x62, 0x75, 0x6d, 0x70, 0x07,
    0x07, 0x01, 0x00, 0x04, 0x6d, 0x69, 0x6e, 0x65,
};

/// Link `importer` against `abi_exporter_wasm` by import order (every import
/// is `(import "e" <name>)`), returning the importer. The exporter is kept
/// alive by the caller (its runtime and arena are what the thunks name).
fn linkAgainstAbiExporter(gpa: Allocator, exporter: *JitInstance, importer_bytes: []const u8, names: []const []const u8) !JitInstance {
    var targets: [8]runner.FuncImportTarget = undefined;
    for (names, 0..) |name, i| {
        targets[i] = exporter.exportedFuncTarget(gpa, name) orelse return error.TestUnexpectedResult;
    }
    return JitInstance.initLinked(gpa, importer_bytes, &.{}, targets[0..names.len], &.{}, &.{});
}

test "JitInstance.initLinked: overflow arguments cross the bridge on every convention (#390)" {
    const gpa = testing.allocator;
    var e = try JitInstance.init(gpa, &abi_exporter_wasm);
    defer e.deinit(gpa);
    var i = try linkAgainstAbiExporter(gpa, &e, &abi_importer_wasm, &.{ "pick7", "sum7", "pick9", "mixsum", "three", "three_of" });
    defer i.deinit(gpa);
    try testing.expectEqual(@as(?u64, 77), try i.invoke(gpa, "test_pick7", &.{}));
    try testing.expectEqual(@as(?u64, 28), try i.invoke(gpa, "test_sum7", &.{}));
    try testing.expectEqual(@as(?u64, 99), try i.invoke(gpa, "test_pick9", &.{}));
    // (1+…+6) + (0.25+0.5+1+…+64) = 21 + 127.75 = 148.75; × 4 = 595.
    try testing.expectEqual(@as(?u64, 595), try i.invoke(gpa, "test_mixsum", &.{}));
}

test "JitInstance.initLinked: a MEMORY-class result crosses the bridge, with and without overflow (#390)" {
    const gpa = testing.allocator;
    var e = try JitInstance.init(gpa, &abi_exporter_wasm);
    defer e.deinit(gpa);
    var i = try linkAgainstAbiExporter(gpa, &e, &abi_importer_wasm, &.{ "pick7", "sum7", "pick9", "mixsum", "three", "three_of" });
    defer i.deinit(gpa);
    try testing.expectEqual(@as(?u64, 123), try i.invoke(gpa, "test_three", &.{}));
    try testing.expectEqual(@as(?u64, 147), try i.invoke(gpa, "test_three_of", &.{}));
}

// The indirect routes reach the SAME thunk (`funcptrs[fidx]` = `dispatch[fidx]`,
// `setup.zig`): a table element that is an import, through `call_indirect`,
// and a `ref.func` of the import through `call_ref`. Shape 1 of #390 was
// suspected to survive on these routes; it is one thunk for all three.
test "JitInstance.initLinked: call_indirect and call_ref through an imported table element cross the bridge (#390)" {
    const gpa = testing.allocator;
    var e = try JitInstance.init(gpa, &abi_exporter_wasm);
    defer e.deinit(gpa);
    var i = try linkAgainstAbiExporter(gpa, &e, &indirect_importer_wasm, &.{ "pick7", "three_of" });
    defer i.deinit(gpa);
    try testing.expectEqual(@as(?u64, 77), try i.invoke(gpa, "test_indirect", &.{}));
    try testing.expectEqual(@as(?u64, 147), try i.invoke(gpa, "test_indirect_three", &.{}));
    try testing.expectEqual(@as(?u64, 77), try i.invoke(gpa, "test_ref", &.{}));
}

// #413 — the importer uses a global, calls a cross-module callee that uses
// ITS global, then reads its own. On arm64 the globals base lives in X23,
// which the callee's prologue overwrites and the thunk must restore; a thunk
// that skipped X23 made this read the exporter's global (1001, not 8).
// x86_64 addresses globals through R15, so it cannot fail there; the
// aarch64-macos CI leg is the instrument.
test "JitInstance.initLinked: the importer's globals base survives a cross-module call (#413)" {
    const gpa = testing.allocator;
    var e = try JitInstance.init(gpa, &glob_exporter_wasm);
    defer e.deinit(gpa);
    var i = try linkAgainstAbiExporter(gpa, &e, &glob_importer_wasm, &.{"bump"});
    defer i.deinit(gpa);
    try testing.expectEqual(@as(?u64, 8), try i.invoke(gpa, "test", &.{}));
    try testing.expectEqual(@as(?u64, 9), try i.invoke(gpa, "test", &.{}));
}

// Cross-module EXCEPTION through the bridge: T defines the tag, E imports it
// and throws, I imports the tag and E's func and catches with `try_table`.
// The unwinder resolves each frame's PC to its instance through
// `eh_registry` (ADR-0134 D2), and the thunk frame is a chain link for it
// (D-238 / ADR-0185); this pins that the ADR-0228 thunk frame still walks.
// The instances are registered the way the spec runner registers its own.
...
const eh_exporter_wasm = [_]u8{
    0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x05, 0x01, 0x60,
    0x01, 0x7f, 0x00, 0x02, 0x08, 0x01, 0x01, 0x74, 0x01, 0x74, 0x04, 0x00,
    0x00, 0x03, 0x02, 0x01, 0x00, 0x07, 0x08, 0x01, 0x04, 0x62, 0x6f, 0x6f,
    0x6d, 0x00, 0x00, 0x0a, 0x08, 0x01, 0x06, 0x00, 0x20, 0x00, 0x08, 0x00,
    0x0b, 0x00, 0x0b, 0x04, 0x6e, 0x61, 0x6d, 0x65, 0x0b, 0x04, 0x01, 0x00,
    0x01, 0x74,
};

const eh_importer_wasm = [_]u8{
    0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x09, 0x02, 0x60,
    0x01, 0x7f, 0x00, 0x60, 0x00, 0x01, 0x7f, 0x02, 0x11, 0x02, 0x01, 0x74,
    0x01, 0x74, 0x04, 0x00, 0x00, 0x01, 0x65, 0x04, 0x62, 0x6f, 0x6f, 0x6d,
    0x00, 0x00, 0x03, 0x02, 0x01, 0x01, 0x07, 0x08, 0x01, 0x04, 0x74, 0x65,
    0x73, 0x74, 0x00, 0x01, 0x0a, 0x15, 0x01, 0x13, 0x00, 0x02, 0x7f, 0x1f,
    0x40, 0x01, 0x00, 0x00, 0x00, 0x41, 0x2a, 0x10, 0x00, 0x0b, 0x41, 0x7f,
    0x0f, 0x0b, 0x0b, 0x00, 0x1c, 0x04, 0x6e, 0x61, 0x6d, 0x65, 0x01, 0x07,
    0x01, 0x00, 0x04, 0x62, 0x6f, 0x6f, 0x6d, 0x03, 0x06, 0x01, 0x01, 0x01,
    0x00, 0x01, 0x68, 0x0b, 0x04, 0x01, 0x00, 0x01, 0x74,
};

test "JitInstance.initLinked: a cross-module throw reaches the importer's try_table through the bridge (registered instances)" {
    const gpa = testing.allocator;
    const reg = runner.eh_registry;
    var t = try JitInstance.init(gpa, &eh_tag_wasm);
    defer t.deinit(gpa);
    const tag = t.exportedTagTarget(gpa, "t") orelse return error.TestUnexpectedResult;
    var e = try JitInstance.initLinked(gpa, &eh_exporter_wasm, &.{}, &.{}, &.{tag}, &.{});
    defer e.deinit(gpa);
...
// An UNRESOLVED import (zero target — the slot keeps `hostDispatchTrap`) must
// trap when called, whatever its signature. `hostDispatchTrap` reads the
// runtime from entry-arg0; for a MEMORY-class callee the x86_64 call site
// puts the hidden result pointer there and the runtime in arg1 (ADR-0228,
// same as a same-module call), so the stub for that shape has to read arg1
// — or it writes `trap_flag` into the result buffer and returns "success".
const unresolved_importer_wasm = [_]u8{
    ,
};

test "JitInstance.initLinked: an unresolved import traps whatever its signature, MEMORY-class included" {
    const gpa = testing.allocator;
    var i = try JitInstance.initLinked(gpa, &unresolved_importer_wasm, &.{}, &.{ .{}, .{} }, &.{}, &.{});
    defer i.deinit(gpa);
    try testing.expectError(error.Trap, i.invoke(gpa, "test_one", &.{}));
    try testing.expectError(error.Trap, i.invoke(gpa, "test_three", &.{}));
}

// A SAME-module `call_ref` whose arguments overflow the registers, in a
// function with no other call: the outgoing-region prescan
// (`emit_setup.computeOutgoingMaxBytes`) did not count `call_ref`, so nothing
// was reserved for the overflow words and, under Win64, the per-call shadow
// SUB moved them out from under the callee. Found by the cross-module funcref
// test on the Windows leg; SysV happened to pass. Windows and arm64 are the
// instruments for this one.
const callref_local_wasm = [_]u8{
    ,
};

test "JIT call_ref: overflow arguments reach the callee when call_ref is the function's only call" {
    const gpa = testing.allocator;
    var i = try JitInstance.init(gpa, &callref_local_wasm);
    defer i.deinit(gpa);
    try testing.expectEqual(@as(?u64, 77), try i.invoke(gpa, "test_ref7", &.{}));
    try testing.expectEqual(@as(?u64, 99), try i.invoke(gpa, "test_ref9", &.{}));
}
